function model_svm = mt_hasvm_coss(param, labels, data, domain_index)
% Cost-Sensitive SSVM
% Usage:
% Input:
% Output:
...
for i=1:size(data,1)
    ...
    write_feat_multiclass(dataid, model_id, labels(i), data(i,:), param, domain_index(i));
end
...
for i=1:num_class
    ...
    if y == i % belief
        if domain_id == 1 % target domain sample, correct prediction
            is_belief = true;
            loss = 0;
        else % source domain sample, correct prediction
            is_belief = true;
            loss = param.gamma;
        end
