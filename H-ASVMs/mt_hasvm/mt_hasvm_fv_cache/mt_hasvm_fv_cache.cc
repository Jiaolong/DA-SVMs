...
static void free_handler(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    for(unsigned int i=0; i< gctx.M_Tree.size(); i++)
        gctx.M_Tree[i].free();
    gctx.M_Tree.clear();
...
static void init_handler(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    // prhs[4]    number of models

    checkM(nrhs == 5, "Expected 4 inputs");
    ...
    const int num_models = (int)mxGetScalar(prhs[4]);
    ...
    // but leaves it uninitialized. It only affects capacity(), but size() will be unchanged.
    gctx.M_Tree.resize(num_models);

    // mexPrintf("Reserved %d models.\n", num_models);
...
static void add_handler(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    // matlab inputs
    //
    //  prhs[1]   key (binary label, dataid, x, y, scale, model_id) (int32)
...
static void gradient_handler(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    const mxArray *mx_full_w = prhs[1];
    const double *full_w     = (const double *)mxGetPr(mx_full_w);
    int num_threads          = (int)mxGetScalar(prhs[2]);

    num_threads = max(1, num_threads);
    omp_set_num_threads(num_threads);

    Moldes &M_Tree    = gctx.M_Tree;

    int dim_full = 0;
    int dim_m = 0;
    // Update the models with the current parameters
    for(unsigned int j=1; j < M_Tree.size(); j++) {
        // m start from 1, the index 0 is default for Ws, the source model
        const double *cur_w   = full_w + dim_full;
        ...
        M.w_hist.pop_back();
        if (w_oldest == NULL)
            w_oldest = new (nothrow) double[dim_m];
        check(w_oldest != NULL);

        // Replace with current w, and put at back of w_hist
        copy(cur_w, cur_w+dim_m, w_oldest);
    ...
    mx_grad = mxCreateNumericArray(1, &dim_full, mxDOUBLE_CLASS, mxREAL);
    grad = mxGetPr(mx_grad);

    double *grad_m   = grad;
    for(unsigned int i=1; i < M_Tree.size(); i++) {
        double obj_val_m = 0;
        gradient(&obj_val_m, grad_m, dim_m, gctx.E, M_Tree, num_threads, i);
        obj_val += obj_val_m;
        grad_m  += dim_m;
    }
...
static void info_handler(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    // Info fields
    enum { SCORE = 0, UNIQUE, DATA_ID, X, Y, SCALE, MODEL_ID, BYTE_SIZE,
    ...
    compute_info(gctx.E, gctx.F, gctx.M_Tree);
    ...
    for (fv_iter i = gctx.F.begin(), i_end = gctx.F.end(); i != i_end; ++i) {
        ...
        *(info + dims[0]*MODEL_ID)  = i->key[fv::KEY_MODEL_ID];
...
static void get_model_handler(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    const int id = (int)mxGetScalar(prhs[1]);
    const Moldes &M_Tree = gctx.M_Tree;
    const model &M = M_Tree[id];
...
static void set_model_handler(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    // prhs[6]    C
    // prhs[7]    model id
    // prhs[8]    model parent_id
    // prhs[9]    any value => quiet mode

    checkM(nrhs >= 9, "Expected >= 8 inputs");
    checkM(nlhs == 0, "Expected 0 outputs");


    int id = (int)mxGetScalar(prhs[7]);
    int parent_id = (int)mxGetScalar(prhs[8]);
    model &M = gctx.M_Tree[id];
    ...
    if (!quiet) {
        mexPrintf("Model id = %d, parent id = %d\n", M.m_id, M.parent_id);
    ...
    // mexPrintf("Set Model: Model id = %d, parent id = %d, C= %f.\n", gctx.M_Tree[id].m_id, gctx.M_Tree[id].parent_id, gctx.M_Tree[id].C);
...
static void obj_val_handler(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    double terms[3];
    for(unsigned int i = 1; i < gctx.M_Tree.size(); i++) { // start from index 1, ignore the source model
        double t_m[3];
        obj_val(t_m, gctx.E, gctx.M_Tree, i);
        terms[0] += t_m[0];
        terms[1] += t_m[1];
        terms[2] += t_m[2];
    }
