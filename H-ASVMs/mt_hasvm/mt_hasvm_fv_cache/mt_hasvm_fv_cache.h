#ifndef MT_HASVM_FV_CACHE_H
#define MT_HASVM_FV_CACHE_H
...
struct fv {
  ...
  enum { KEY_DATA_ID = 0, // Source image id
         ...
         KEY_Y,           // y location
         KEY_SCALE,       // scale
         KEY_MODEL_ID,    // hierarchical model id
         KEY_LEN };
  ...
  void print() {
    mexPrintf("dataid: %d  x: %d  y: %d  "
              "scale: %d  &feat: %x, num_blocks: %d, block_labels[0]: %d, block_labels[9]: %d, uniq: %d loss: %f\n", 
              key[KEY_DATA_ID], key[KEY_X], key[KEY_Y], 
              key[KEY_SCALE], feat, num_blocks, block_labels[0], block_labels[9], is_unique, loss);
  }
